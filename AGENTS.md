...
- **完了済み legacy 計画**: `docs/archive/2026Q1/server-modernization/planning/server_modernization_wbs_detailed.md` は archive 参照専用とする。
  - 当面の `server-modernized` 作業判断は `docs/DEVELOPMENT_STATUS.md`・`AGENTS.md`・最新指示・task 別実施記録を優先する。
...
## 4. ワーキングフロー (作業手順)
1. **理解**: 指示内容と `docs/DEVELOPMENT_STATUS.md` を確認する。`server-modernized` の legacy 計画履歴が必要な場合のみ archive 済み WBS を参照する。
...
## 7. サブエージェントルール 
- サブエージェントが使用可能な場合は積極的にサブエージェントを動かして、自分のコンテキストを圧迫しないような運用を心がけること。
