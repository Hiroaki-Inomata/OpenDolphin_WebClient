package open.dolphin.shared.legacytouch;
...
public class LegacyTouchAbstractResource {
    private static final Logger LOGGER = Logger.getLogger(LegacyTouchAbstractResource.class.getName());
    ...
    protected static String sexValueToDesc(String code) {
        for (String male : MALES) {
            if (code.startsWith(male)) {
                return MALE;
            }
        }
        for (String female : FEMALES) {
            if (code.startsWith(female)) {
                return FEMALE;
            }
        }
        return UNKNOWN;
    ...
    protected void writeChildren(StringBuilder sb, org.jdom.Element current) {
        ...
        if (eName.equals(PARAGRAPH_NAME)) {
            eType = TT_PARAGRAPH;
            startParagraph(sb, current.getAttributeValue(LOGICAL_STYLE_NAME), current.getAttributeValue(ALIGNMENT_NAME));
            ...
            eType = TT_COMPONENT;
            startComponent(sb, current.getAttributeValue(NAME_NAME), current.getAttributeValue(COMPONENT_ELEMENT_NAME));
    ...
    protected String toIsoPvtDate(LocalDateTime pvtDate) {
        return pvtDate != null ? ModelUtils.formatDateTime(pvtDate) : null;
    }
}
