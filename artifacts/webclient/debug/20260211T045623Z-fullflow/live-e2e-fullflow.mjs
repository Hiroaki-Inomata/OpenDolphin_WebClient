...
const normalizePatientNumericId = (value) => {
...
const main = async () => {
    ...
    async () => {
      ...
      if (page.url().includes('/reception')) {
        ...
        try {
          ...
          await page.goto(`${BASE_URL}/f/${encodeURIComponent(FACILITY_ID)}/reception`, {
  ...
  await runStep(
    ...
    async () => {
      ...
      const targetPatientNumericId = 1;
      let selected = false;
      for (let attempt = 0; attempt < 3 && !selected; attempt += 1) {
        for (const candidate of candidateIds) {
          ...
          const rows = page.locator('.reception-patient-search__item');
          const count = await rows.count();
          for (let i = 0; i < count; i += 1) {
            const row = rows.nth(i);
            if (!(await isVisible(row, 800))) continue;
            const text = (await row.textContent()) ?? '';
            const match = text.match(/患者ID:\s*([0-9]+)/);
            if (!match?.[1]) continue;
            const numeric = normalizePatientNumericId(match[1]);
            if (numeric !== targetPatientNumericId) continue;
            const selectButton = row.locator('.reception-patient-search__item-select').first();
            if (!(await isVisible(selectButton, 1000))) continue;
  ...
  await runStep(
    ...
    async () => {
      ...
      const payloadApiResult =
        payload && typeof payload === 'object' && typeof payload.apiResult === 'string' ? payload.apiResult : null;
      if (await isVisible(apiResultLocator, 10_000)) {
        receptionApiResult = (await apiResultLocator.innerText()).trim();
      } else if (payloadApiResult) {
        receptionApiResult = `Api_Result: ${payloadApiResult}`;
      } else {
        receptionApiResult = null;
      }

      const toneBanner = page.locator('.reception-accept__result .tone-banner').first();
      const toneMessage = (await isVisible(toneBanner, 3000))
        ? (await toneBanner.innerText()).trim()
        : (payload && typeof payload === 'object' && typeof payload.apiResultMessage === 'string' ? payload.apiResultMessage : '');
      ...
      if (!receptionApiResult || !/Api_Result:\s*(00|0000|K3|16)/.test(receptionApiResult)) {
  ...
  await runStep('病名入力（追加）', async () => {
    ...
    if (!saved) {
      ...
      return { saved: false, reflected: false, note: '保存表示の確認は未達（UI反映待ち/表示揺れ）' };
    }
    await takeScreenshot(page, '06-diagnosis-added.png');
    return { saved: true, reflected: true };
  });

  await runStep('SOAP全欄記載と保存（Free入力確認）', async () => {
    ...
    if (await isVisible(feedback, 5000)) {
      ...
      const errorBanner = soapPanel.locator('.tone-banner--error').first();
      if (await isVisible(errorBanner, 1200)) {
        const msg = ((await errorBanner.textContent()) ?? '').trim();
        throw new Error(`SOAP保存エラー表示: ${msg || 'unknown'}`);
      }
      feedbackText = 'feedback-missing';
    }

    for (let i = 0; i < 4; i += 1) {
      const freeField = page.locator('#soap-note-free').first();
      if (await isVisible(freeField, 300)) break;
      const cycleButton = soapPanel.locator('.soap-note__actions button', { hasText: /^表示:/ }).first();
      if (!(await isVisible(cycleButton, 300))) break;
      await cycleButton.click();
      await page.waitForTimeout(250);
    }

    const freeValue = await page.locator('#soap-note-free').first().inputValue().catch(() => '');
    const freeTextVisible = await isVisible(page.getByText(freeToken).first(), 1200);
    if (!freeValue.includes(freeToken) && !freeTextVisible) {
      throw new Error('Free入力の保持を確認できません。');
    }

    await takeScreenshot(page, '07-soap-saved.png');
    return { feedback: feedbackText, freeRetained: true, freeToken };
  }, { critical: true });
  ...
  await runStep('症状詳記入力と登録', async () => {
    ...
    await foldSummary.click({ force: true });
  ...
  const saveOrderTab = async ({ action, entity, label }) => {
    ...
    const reflected = await isVisible(bundleInList, 5000);

    await takeScreenshot(page, `09-order-${sanitize(action)}.png`);
    return { action, entity, bundleLabel, reflected };
  };
  ...
  await runStep('診療終了', async () => {
    ...
    const toastText = (await isVisible(toast, 1000)) ? (await toast.innerText()).trim() : null;
    await takeScreenshot(page, '11-finish.png');
    return { skipped: true, reason: 'ORCA送信優先のため本ランでは診療終了クリックを省略', toast: toastText };
  });
  ...
  await runStep(
    ...
    async () => {
      await ensureActionBarExpanded(page);
      const sendButtonById = page.locator('#charts-action-send').first();
      const sendButtonByRole = page.getByRole('button', { name: 'ORCA 送信' }).first();
      const sendButton = (await isVisible(sendButtonById, 1500)) ? sendButtonById : sendButtonByRole;
      ...
      const waitUntilEnabled = async () => {
        ...
        while (Date.now() - start < 60_000) {
          if (!(await sendButton.isDisabled())) return true;
          const reason = await sendButton.getAttribute('data-disabled-reason');
          if (reason && reason !== 'running') break;
          await page.waitForTimeout(800);
        }
      ...
      if (await sendButton.isDisabled()) {
        const enabled = await waitUntilEnabled();
        if (!enabled) {
          const currentToast = page.locator('.charts-actions__toast').first();
          if (await isVisible(currentToast, 1500)) {
            const text = (await currentToast.innerText()).trim();
            if (/ORCA送信を完了/.test(text)) {
              sendToastText = text;
              await takeScreenshot(page, '12-orca-send-success.png');
              return { status: null, toast: sendToastText, invoiceNumber: null, dataId: null, alreadySent: true };
            }
          }
          const reason = await sendButton.getAttribute('data-disabled-reason');
    ...
    async () => {
      ...
      if (await isVisible(billingButton, 5000)) {
        try {
          await Promise.all([
            page.waitForURL((url) => url.pathname.includes('/reception'), { timeout: 60_000 }),
            billingButton.click({ force: true }),
          ]);
        } catch {
