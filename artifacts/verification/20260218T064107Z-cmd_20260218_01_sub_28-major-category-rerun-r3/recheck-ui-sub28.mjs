...
const clickVisibleCategoryButton = async (page, label) => {
  const buttons = page.locator('#charts-utility-pane button').filter({ hasText: label });
...
page.on('requestfailed', (request) => {
  ...
  masterResponses.push({ type: 'requestfailed', url: request.url(), method: request.method(), error: request.failure()?.errorText ?? null, at: new Date().toISOString() });
});
