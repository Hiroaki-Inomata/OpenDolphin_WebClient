...
- 補足: ORCA モダナイズ REST（/orca/master/*, /orca/tensu/*）の最新サマリは `src/webclient_modernized_bridge/03_ギャップ解消方針とUI影響分析.md` §8（RUN_ID=`20251123T135709Z`）を参照。
...
### OrcaPatientApiResource / OrcaBridgeResource（P2-06後）
| HTTP | パス | 主な処理 | 備考 |
| --- | --- | --- | --- |
| GET | `/api01rv2/patientgetv2` | 患者参照（JSON） | `/orca/patientgetv2` `/api/orca/patientgetv2` `/api/api01rv2/patientgetv2` も同義。`format=json` のみ許可。 |
| POST | `/api/v1/orca/bridge` | ORCA XML2 ブリッジ実行（JSON契約） | 旧 XML POST は web-client 側で本エンドポイントへ自動転送。 |

> 2026-03-11（RUN_ID=20260311T060146Z, P2-06）で以下の XML 専用公開Resourceを削除済み:  
> `OrcaAcceptanceListResource` `OrcaSystemManagementResource` `OrcaReportResource` `OrcaDiseaseApiResource` `OrcaMedicalApiResource` `OrcaAdditionalApiResource`
...
- デモモード用エンドポイント。`/touch` 相当の読み取り専用 API を提供。
- 現行実装: JSON は `open.dolphin.rest.DemoResourceAsp` に集約済み。XML 版 `DemoResource`/`DemoResourceASP` の公開は終了（`web.xml` から登録削除）。【F:server-modernized/src/main/java/open/dolphin/rest/DemoResourceAsp.java†L85-L825】【F:server-modernized/src/main/java/open/dolphin/touch/DemoResource.java†L24-L347】【F:server-modernized/src/main/java/open/dolphin/touch/DemoResourceASP.java†L24-L1440】
- 利用判断の一次情報: `src/server_modernized_gap_20251221/04_touch_demo/Demo_API_整理.md`
...
>
> **2025-12-24 利用確認 (RUN_ID: 20251224T163500Z)**: Web クライアント/非 Phase2 ドキュメントに `/demo` の利用記述が見当たらず、15 API は「保留（決裁待ち）」として整理。
>
> **2025-12-25 合意 (RUN_ID: 20251225T195836Z)**: JSON 版 15 API を維持し、XML 版 `DemoResource`/`DemoResourceASP` の公開を終了（web.xml から登録削除）。

#### Demo エンドポイント一覧（2025-12-25 / RUN_ID: 20251225T195836Z）

| ID | HTTP | パス | 機能概要 | 実装形態 | 運用判断 |
| --- | --- | --- | --- | --- | --- |
| Demo-01 | GET | `/demo/document/progressCourse/{param}` | デモカルテの経過記録 XML を返し、看護業務フローを再現。 | JSON: `DemoResourceAsp` | 維持（JSON）/ XML 廃止 |
| Demo-02 | GET | `/demo/item/laboItem/{param}` | 単項目ラボトレンドを返却しグラフ表示を再現。 | JSON: `DemoResourceAsp` | 維持（JSON）/ XML 廃止 |
| Demo-03 | GET | `/demo/module/diagnosis/{param}` | 登録診断のページング取得。 | JSON: `DemoResourceAsp` | 維持（JSON）/ XML 廃止 |
| Demo-04 | GET | `/demo/module/laboTest/{param}` | 検査結果モジュール一覧を返却。 | JSON: `DemoResourceAsp` | 維持（JSON）/ XML 廃止 |
| Demo-05 | GET | `/demo/module/rp/{param}` | 処方モジュールを返却。 | JSON: `DemoResourceAsp` | 維持（JSON）/ XML 廃止 |
| Demo-06 | GET | `/demo/module/schema/{param}` | スキーマ画像を Base64 で返却。 | JSON: `DemoResourceAsp` | 維持（JSON）/ XML 廃止 |
| Demo-07 | GET | `/demo/module/{param}` | 任意 entity のモジュール（観察値等）取得。 | JSON: `DemoResourceAsp` | 維持（JSON）/ XML 廃止 |
| Demo-08 | GET | `/demo/patient/firstVisitors/{param}` | 初診患者一覧を返却。 | JSON: `DemoResourceAsp` | 維持（JSON）/ XML 廃止 |
| Demo-09 | GET | `/demo/patient/visit/{param}` | 来院履歴を返却。 | JSON: `DemoResourceAsp` | 維持（JSON）/ XML 廃止 |
| Demo-10 | GET | `/demo/patient/visitLast/{param}` | 最終来院情報を返却。 | JSON: `DemoResourceAsp` | 維持（JSON）/ XML 廃止 |
| Demo-11 | GET | `/demo/patient/visitRange/{param}` | 期間指定の来院履歴を返却。 | JSON: `DemoResourceAsp` | 維持（JSON）/ XML 廃止 |
| Demo-12 | GET | `/demo/patient/{pk}` | 患者基本情報を取得。 | JSON: `DemoResourceAsp` | 維持（JSON）/ XML 廃止 |
| Demo-13 | GET | `/demo/patientPackage/{pk}` | 患者パッケージ（患者＋保険＋アレルギー）取得。 | JSON: `DemoResourceAsp` | 維持（JSON）/ XML 廃止 |
| Demo-14 | GET | `/demo/patients/name/{param}` | 氏名検索で患者リストを返却。 | JSON: `DemoResourceAsp` | 維持（JSON）/ XML 廃止 |
| Demo-15 | GET | `/demo/user/{param}` | デモユーザーの資格情報を返却。 | JSON: `DemoResourceAsp` | 維持（JSON）/ XML 廃止 |

※ デモ運用の一次情報が入手できた場合は、JSON 版 15 API の維持範囲を再評価する。
RUN_ID: 20251225T195836Z
...
| --- | --- | --- | --- | --- | --- | --- |
| 2 | P0 | `/orca14/appointmodv2` | `POST /orca/appointments/mutation` | ◎ 実装完了 | RUN_ID=20251121PostOpenCheckZ1 実測（`docs/server-modernization/phase2/operations/logs/20251121PostOpenCheckZ1-api-contract.md#orca14appointmodv2`） | 予約登録/取消/新規患者番号割当。`AppointmentMutationRequest/Response` と `OrcaAppointmentMutationFacade` で `AppoServiceBean` を同期し、警告は SSE へ連携。2025-11-21 開発接続先（詳細は `docs/web-client/operations/mac-dev-login.local.md` 参照）では `HTTP 405 (Allow: OPTIONS, GET)` のまま（`20251121T153200Z` → `20251121PostOpenCheckZ1` で変化なし）。Evidence: `artifacts/orca-connectivity/20251121PostOpenCheckZ1/crud/appointmodv2/headers_*.txt`。 |
| 4 | P0 | `/orca11/acceptmodv2` | `POST /orca/visits/mutation` | ◎ 実装完了 | RUN_ID=20251121PostOpenCheckZ1 実測（`docs/server-modernization/phase2/operations/logs/20251121PostOpenCheckZ1-api-contract.md#orca11acceptmodv2`） | 受付 CRUD・新規患者受付。`VisitMutationRequest/Response` で `PVTServiceBean` と `ChartEventServiceBean` を同時更新し、`PatientServiceBean` が番号確定を補助。2025-11-21 ORMaster も `HTTP 405 (Allow: OPTIONS, GET)` のまま（`20251121T153200Z` → `20251121PostOpenCheckZ1` で変化なし）。開放後に再検証する。Evidence: `artifacts/orca-connectivity/20251121PostOpenCheckZ1/crud/acceptmodv2/headers_*.txt`。 |
| 6 | P0 | `/api01rv2/appointlstv2` | `POST /orca/appointments/list` | ◎ 実装完了 | RUN_ID=20251121T153200Z 実測（`docs/server-modernization/phase2/operations/logs/20251121T153200Z-api-contract.md#appointlstv2`） | `OrcaAppointmentResource#listAppointments` が ORCA XML を DTO `OrcaAppointmentListResponse` へ変換し、予約ウィジェットがモダナイズ REST だけで取得できるようにした。2025-11-21 の ORMaster 実測では `Api_Result=12 (ドクターが存在しません)` でデータ不足（Evidence: `crud/appointlstv2/response_*.xml`）。医師マスタ投入後に再測予定。 |
...
| 42 | P2 | `/orca42/receiptprintv3` | `POST /orca/report/print` | ◎ 実装完了 | RUN_ID=20251121PostOpenCheckZ1 実測（`docs/server-modernization/phase2/operations/logs/20251121PostOpenCheckZ1-api-contract.md#orca42receiptprintv3`） | 帳票印刷ジョブ（push-exchanger 連携 + `/blobapi/{Data_Id}` ダウンロード）。2025-11-21 ORMaster も `HTTP 405 (Allow: OPTIONS, GET)` のため POST 不可（前回 `20251121T153200Z` から変化なし）。push-exchanger/`/blobapi` 構成後に再検証。Evidence: `artifacts/orca-connectivity/20251121PostOpenCheckZ1/crud/receiptprintv3/headers_*.txt`。 |
