...
- 更新日: 2026-01-30
- 更新: 2026-02-13（RUN_ID: 20260213T114440Z）
...
└─ Left（一覧・操作エリア）
   ...
   ├─ 例外一覧（未承認/送信エラー/遅延）
   └─ ステータス別リスト（予約/診察待ち/診察中/診察終了/会計済み）
...
### 3.5 ステータス別リスト（予約/診察待ち/診察中/診察終了/会計済み）
- **構成**:
...
- **行操作**:
  ...
  - 診察終了カード: 個別の「会計送信」ボタンで ORCA `medicalmodv2` 送信 → 成功時は会計済みへ移動
