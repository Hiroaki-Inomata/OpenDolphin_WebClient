...
- `npm run verify:no-public-secrets` は `web-client` 配下の `.env*` を再帰走査し、秘密名キーワードを含む `VITE_` キーを拒否します。
- `.env.local` / `.env.*` に機密情報を記載した場合はリポジトリへコミットせず、Secret Manager 等へ移管してください。
